...
from src.utils import ensure_dir, load_checkpoint, resolve_device, write_json
...
def main() -> None:
    ...
    load_image_classifier_state(model_cpu, payload['model_state_dict'])
    model_cpu.eval()
    ...
    checkpoint_val_acc = payload.get('metrics', {}).get('val_acc')
    ...
    write_json(
        output_dir / f'{args.split}_summary.json',
        {
            ...
            'device': str(device),
            'ranks': ranks,
            'best_checkpoint_val_acc': None if checkpoint_val_acc is None else float(checkpoint_val_acc),
